...
Add `swift-google-cloud-storageinsights-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-cloud-storageinsights-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudStorageInsightsV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudStorageInsightsV1 <target-name> --package swift-google-cloud-storageinsights-v1
```
